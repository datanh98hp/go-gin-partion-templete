...
const (
	...
	RefreshTokenTTL = 1 * 24 * time.Hour // 1 day
)
...
func (js *JWTService) DecryptAccessTokenPayload(tokenString string) (*EncryptedPayload, error) {
	...
	if err != nil {
		return nil, utils.NewWrapError("Cannot parse token", utils.ErrorCodeInternal, err)
	}
	...
	if err != nil {
		return nil, utils.NewWrapError("Cannot decode data", utils.ErrorCodeInternal, err)
	}

	var payload EncryptedPayload
	if err := json.Unmarshal(decryptedBytes, &payload); err != nil {
		return nil, utils.NewWrapError("Invalid data format", utils.ErrorCodeInternal, err)
	}
...
func (js *JWTService) ValidateRefreshToken(token string) (RefreshToken, error) {
	...
	if err != nil || refreshToken.Revoked || refreshToken.ExpiresAt.Before(time.Now()) {
		return RefreshToken{}, utils.NewWrapError("Cannot get refresh token", utils.ErrorCodeInternal, err)
	}
...
func (js *JWTService) RevokeRefreshToken(token string) error {
	...
	err := js.cache.Get(cacheKey, &refreshToken)
	if err != nil {
		return utils.NewWrapError("Cannot get refresh token", utils.ErrorCodeInternal, err)
	}
