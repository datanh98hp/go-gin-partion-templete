...
func NewMailService(cfg *config.Config, logger *zerolog.Logger, providerFactory ProviderFactory) (EmailProviderService, error) {
	...
	if err != nil {
		return nil, utils.NewWrapError("Failed to create provider", utils.ErrorCodeInternal, err)
	}
...
func (ms *MailService) SendMail(ctx context.Context, email *Email) error {
	...
	return utils.NewWrapError("Failed to send email after all retries", utils.ErrorCodeInternal, lastErr)
