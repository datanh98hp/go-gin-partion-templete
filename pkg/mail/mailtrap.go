...
func NewMailtrapProvider(config *MailConfig) (EmailProviderService, error) {
	...
	if !ok {
		return nil, utils.NewError("Invalid or missing MailTrap configuaration", utils.ErrorCodeInternal)
	}
...
func (p *MailtrapProvider) SendMail(ctx context.Context, email *Email) error {
	...
	if err != nil {
		return utils.NewWrapError("Failed to marshal email", utils.ErrorCodeInternal, err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, p.config.MailTrapUrl, bytes.NewReader(payload))
	if err != nil {
		return utils.NewWrapError("Failed to create request", utils.ErrorCodeInternal, err)
	}
	...
	if err != nil {
		...
		return utils.NewWrapError("Failed to send request", utils.ErrorCodeInternal, err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		log.Printf("---- email:%v", resp.StatusCode)
		return utils.NewError(fmt.Sprintf("Unexpected response from mailtrap with code %d: %s", resp.StatusCode, string(body)), utils.ErrorCodeInternal)
	}
